    ...
    return (
        <View style={styles.container}>
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                ...
                {type !== "devis" ?
                    <View>
                        <Text style={{ textDecorationLine: "underline", color: colors.black }}>{type === "facture" ? "FACTURE" : type === "devis" ? "DEVIS" : "ISAGO"}</Text>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                {type !== "devis" ? <View style={{ flexDirection: "row", alignItems: "center", gap: 5 }}>
                    ...
                    <Text style={{ color: colors.black }}>{facture?.status === "PENDING" ? "En attente de confirmation" : facture?.status === "PAID" ? "Paiement réussi" : "Paiement annulé"}</Text>
                </View> :
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 5 }}>
                        ...
                        <Text style={{ color: colors.black }}>{(facture?.status === "PENDING" && facture?.paymentStatus !== "PAID") ? "En attente de confirmation" : (facture?.status === "REJECT" && facture?.paymentStatus !== "PAID") ? "Paiement annulé" : facture?.paymentStatus === "PAID" && "Paiement réussi"}</Text>
                    </View>
                }
