    ...
    return (
        <TouchableOpacity onPress={() => navigation.navigate("devis_details", { devis: devi })} style={styles.container}>
            <View style={{ flexDirection: 'row', justifyContent: "space-between", alignItems: "baseline" }}>
                <Text style={{ fontSize: 20, fontWeight: "bold", textDecorationLine: "underline" }}>{devi?.typeCompteur}</Text>
                {devi?.status === "VALIDATED" && <View>
                    <Text style={{ fontSize: 12 }}>Montant</Text>
                    <Text style={{ fontStyle: "italic", color: colors.red, alignSelf: "flex-end", fontSize: 13 }}>{devi?.amount}  fcfa</Text>
                </View>}
            </View>

            <Text>A usage <Text style={{ fontWeight: "bold", letterSpacing: 1 }}>{devi?.usage}</Text></Text>

            {devi?.typeDemande === "Nouveau" && <Text>Demande de <Text style={{ fontWeight: "bold", letterSpacing: 1, color: "brown", textDecorationLine: "underline" }}>{devi?.typeDemande}</Text> compteur</Text>}
            {devi?.typeDemande === "Mutation/Aug./Dim./Chang. Puissance" && <Text>Demande de <Text style={{ fontWeight: "bold", letterSpacing: 1, color: "brown", textDecorationLine: "underline" }}>Changement de puissance</Text></Text>}
            {devi?.typeDemande === "Réabonnement" && <Text>Demande de <Text style={{ fontWeight: "bold", letterSpacing: 1, color: "brown", textDecorationLine: "underline" }}>{devi?.typeDemande}</Text> de compteur</Text>}
            ...
            {devi?.status && <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", width: "100%", }}>
                <View style={{ borderRadius: 5, alignItems: "center", justifyContent: 'center', paddingVertical: 5 }}>
                    {devi?.paymentStatus === "PAID" && <Text style={{ color: colors.success, textAlign: "center" }}>Devis payé.</Text>}
                    {devi?.status === "VALIDATED" && devi?.paymentStatus !== "PAID" && <Text style={{ color: colors.success, textAlign: "center" }}>Devis validé.</Text>}
                    ...
                    {devi?.status === "REJECT" && devi?.paymentStatus !== "PAID" && <View style={{ flexDirection: "row" }}>
                        <Text style={{ color: colors.danger, textAlign: "center" }}>Devis rejeté.</Text>
                    </View>}

                </View>

                <Text style={{ fontSize: 10 }}>{moment(devi?.updatedAt).fromNow()}</Text>
            </View>
            }
